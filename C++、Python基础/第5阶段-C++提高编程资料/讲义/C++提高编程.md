...
* `erase(elem);`            ==//删除容器中值为elem的元素。类似list中的remove==
...
* ==map插入方式很多，记住其一即可，参考上面程序中的四种示例==
  * 第二种比较好，不需要设置数据类型
  * 第四种不建议插入数据，一般用来根据key查找value
...
#### 5.1.1 for_each——学习普通函数和函数对象的写法区别
...
void test01() {
	...
	//遍历算法
	for_each(v.begin(), v.end(), print01); // 注意这里只放函数名即可
	cout << endl;

	for_each(v.begin(), v.end(), print02()); // 注意这里是带（）的函数对象，指匿名函数对象
	cout << endl;
}
...
==有序就是已经排好序（升序或者降序）的数据==，可以和set和map结合使用
...
**总结：**二分查找法查找效率很高，值得注意的是查找的容器中元素==必须是有序序列==
...
**总结：**merge合并的两个容器==必须的有序序列==，结果也是有序序列
...
**总结：**replace_if按条件查找，可以==利用仿函数灵活筛选==满足的条件
...
void test01()
{
    ...
    // 5 6 7 8 9
    // 如果用vTarget.end()则输出为 5 6 7 8 9 0 0 0 0 0，此时会打印所有的元素，然而实际的交集不会有这么多
...
==v1和v2的差集，在v1中不再v2中的数；v2和v1的差集，在v2中不再v2中的数==
